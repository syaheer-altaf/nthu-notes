...
\begin{document}
	...
	The goal of this randomized algorithm is to sample from a complex target distribution via the construction of an \emph{ergodic}, \emph{time-reversible} Markov chain whose stationary distribution \emph{is} the target distribution. Recall that a finite Markov chain is called ergodic if it is irreducible and aperiodic. Informally, it means to say that every state in the chain is reachable and the states do not get trapped in strict repeating cycles of steps.
	We then introduce the definition of a time-reversible Markov chain with some important lemmas to make sense of the algorithm.
	...
	\begin{lemma}
		...
		\]
		Note that if our Markov chain is time-reversible, it is not difficult to see that \(Q_{i,j} = P_{i,j}\) from Definition~\ref{time-reversible}.
		\label{lemma-1}
	\end{lemma}
	\begin{proof}
		...
		\end{align*}
		Suppose the claim holds true for all \(k+1\) states. Then,
		\begin{align*}
			\Pr(x_{m-k-1} \mid x_{m-k}, x_{m-k+1},\dots,x_m) = \frac{\Pr(x_{m-k-1},\dots, x_m)}{\Pr(x_{m-k},\dots, x_m)}
		\end{align*}
		\begin{flalign*}
			= \frac{\Pr(x_m)\Pr(x_{m-1} \mid x_m)\Pr(x_{m-2} \mid x_{m-1})\dots\Pr(x_{m-k} \mid x_{m-k+1})\Pr(x_{m-k-1} \mid x_{m-k},x_{m-k+1})}{\Pr(x_m)\Pr(x_{m-1} \mid x_m)\Pr(x_{m-2} \mid x_{m-1})\dots\Pr(x_{m-k} \mid x_{m-k+1})} &
		\end{flalign*}
		\begin{align*}
			&= \Pr(x_{m-k-1} \mid x_{m-k},x_{m-k+1}) &&(\text{similar argument as base case})\\
			&= \Pr(x_{m-k-1} \mid x_{m-k}).
		\end{align*}
		The second equality comes from the multiplication rule and the induction hypothesis. Thus, we establish that the reversed-sequence chain is indeed Markovian, as it has the memoryless property. Furthermore, we can compute the transition probability by the definition of conditional probability (when the chain started in \emph{stationarity}):
		\begin{align*}
	...
	\begin{lemma}
		Suppose we have a Markov chain with transition matrix \(\vec{P}\) and stationary distribution \(\overline{\pi}\). Suppose also that the chain is time-reversible. Then, the reversed chain also has the same stationary distribution \(\overline{\pi}\).
		\label{time-reverse-lemma}
	\end{lemma}
	\begin{proof}
		Since \(\vec{Q} = \vec{P}\) from Lemma~\ref{lemma-1}, where \(\vec{Q}\) is the reversed chain's transition matrix, suffice it to say that \(\overline{\pi}\) is also the stationary distribution for the reversed chain.
	\end{proof}
	
	From Lemma~\ref{lemma-1} and Lemma~\ref{time-reverse-lemma}, we know that if a Markov chain is time reversible, then the transition matrix and the stationary distribution are the same for forward-time and reverse-time. Let us use this to our advantage in constructing an algorithm that moves from one state to another according to a transition matrix \(\vec{P}\). In particular, suppose we have a target distribution \(\overline{\rho}\) we wish to sample from. We construct an algorithm whose transition matrix satisfies the detailed-balance equation; i.e., \(\rho_iP_{i,j} = \rho_jP_{j,i}\). Consequently, 
	\begin{align*} 
		\sum_i\rho_iP_{i,j} &= \rho_j\sum_i P_{j,i}\\ 
		&= \rho_j. 
	\end{align*} 
	
	This holds true for any \(j\)-th entry of \(\overline{\rho}\cdot\vec{P}\). This proves that \(\overline{\rho}\cdot\vec{P} = \overline{\rho}\). Thus, with the right transition matrix, the constructed chain has the target distribution as the stationary distribution which implies that it is also time-reversible, and hence both lemmas apply. The remaining fact is to ensure ergodicity.
	
	\paragraph{Note.} On a finite state space, \emph{ergodic} usually means \emph{irreducible and aperiodic}.
	Irreducibility implies the existence and uniqueness of a stationary distribution, and aperiodicity then ensures convergence to it from any initial distribution
	(see Theorem~19 of \texttt{My Notes for Discrete Probability: Detailed Explanations on Concepts and Proofs}).
	
	\begin{algorithm}[H]
		\caption{\textsc{Metropolis-Hastings Algorithm (Discrete Version)}}
		\label{alg:metropolis}
		\textsc{Metropolis}(\(\rho\))
		\begin{algorithmic}[1]
			\REQUIRE A target distribution \(\rho\) (typically a PMF). The goal is to sample from this distribution.
			\STATE Define number of steps \(N\).
			\STATE Select an arbitrary state \(x\) such that \(\rho(x) > 0\).
			\FOR{\(i=0,\ldots,N-1\)}
			\STATE Compute \(\rho(x)\).
			\STATE Propose a new state \(y\) from a (discrete) distribution conditional on \(x\); say, \(q(\cdot\mid x)\).
			\STATE Compute \(\rho(y)\).
			\IF{\(U[0,1] < \frac{\rho(y)}{\rho(x)}\cdot\frac{q(x\mid y)}{q(y\mid x)}\)}
			\STATE \(x \gets y\).
			\ENDIF
			\ENDFOR
			\RETURN \(x\).
	...
	\end{algorithm}
	\begin{analysis}
		The analysis here is to show that the algorithm constructs a time-reversible Markov chain with stationary distribution \(\rho\), and that under appropriate conditions on the proposal distribution \(q(\cdot\mid x)\), the chain is ergodic.
		\paragraph{Transition kernel.}
		Define the acceptance probability (from Step~7 of the algorithm)
		\begin{equation}
		\label{eq:acceptance}
			\alpha(x,y) :=\min\Big(1, \frac{\rho(y)}{\rho(x)}\cdot\frac{q(x\mid y)}{q(y\mid x)}\Big).
		\end{equation}
		The transition probabilities induced by the algorithm are
		\begin{equation}
		\label{eq:pxy}
			P_{x,y}=q(y\mid x)\alpha(x,y), \qquad x\neq y,
		\end{equation}
		and
		\begin{equation}
		\label{eq:self_loop}
			P_{x,x} = 1-\sum_{y\neq x}q(y\mid x)\alpha(x,y).
		\end{equation}
		Similarly,
		\begin{equation}
		\label{eq:pyx}
			P_{y,x} = q(x\mid y)\alpha(y,x).
		\end{equation}
		
		\paragraph{Time-reversibility.} We want to show that the detailed-balance equation is satisfied under the transition kernel which implies time-reversibility with \(\rho\) being the stationary distribution from the previous discussion. Observe the following when \(x \neq y\):
		\begin{itemize}
			\item \textbf{Case 1:} \(\alpha(x,y) < 1\) and \(\alpha(y,x) = 1\), and thus from Equations~\ref{eq:acceptance},~\ref{eq:pxy}, and~\ref{eq:pyx}, \(\rho(x)P_{x,y} = \rho(y)\cdot\)\textcolor{red}{\(q(x\mid y)\)}\(=\rho(y)\cdot\)\textcolor{red}{\(P_{y,x}\)}.
			\item \textbf{Case 2:} \(\alpha(x,y) = 1\) and \(\alpha(y,x) < 1\), and thus from Equations~\ref{eq:acceptance},~\ref{eq:pxy}, and~\ref{eq:pyx}, \(\rho(y)P_{y,x} = \rho(x)\cdot\)\textcolor{red}{\(q(y\mid x)\)}\(=\rho(x)\cdot\)\textcolor{red}{\(P_{x,y}\)}.
			\item \textbf{Case 3:} \(\alpha(x,y) = \alpha(y,x) = 1\). In this case, Equation~\ref{eq:acceptance} implies that \(\rho(y)\cdot q(x\mid y) = \rho(x)\cdot q(y\mid x)\). But in Equations~\ref{eq:pxy} and~\ref{eq:pyx}, we get \(P_{x,y} = q(y \mid x)\) and \(P_{y,x} = q(x \mid y)\). Thus, the detailed-balance equation is still satisfied.
		\end{itemize}
		For \(x=y\), the detailed-balance equation is trivial. Thus, the chain is reversible with respect to \(\rho\), and \(\rho\) is stationary.
		
		\paragraph{Ergodicity.} For irreducibility, the proposed distribution \(q(\cdot\mid x)\) must ensure that every support state is reachable; i.e., \(q(\theta \mid x) \neq 0\) when \(\rho(\theta) > 0\). Also, from Equation~\ref{eq:self_loop}, there is a non-zero probability of self-loop (i.e., \(P_{x,x} > 0\) when \(q(x\mid x) \neq 0\)) and this implies aperiodicity. These are the conditions for \(q(\cdot\mid x)\) to induce a chain that is ergodic, albeit a more relaxed version is available. The usual proposed distribution is typically a uniform centered at current state \(x\). In this case, \(q(y\mid x) = q(x\mid y)\) and thus the acceptance probability is simply the ratio \(\min(1, \rho(y)/\rho(x))\).
	\end{analysis}
	
	Algorithm~\ref{alg:metropolis}, as written, still applies when \(\rho\) and \(q(\cdot\mid x)\) are continuous densities. However, its formal analysis requires continuous-state transition kernels and more general notions of ergodicity.
